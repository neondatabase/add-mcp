...
import { buildServerConfig, installServerForAgent } from '../../src/installer.js';
...
test('E2E: Install package MCP to Cursor (local)', () => {
  ...
  assert.deepStrictEqual(serverConfig.args, ['-y', '@modelcontextprotocol/server-postgres']);
});
...
test('E2E: Install command MCP to Claude Code (local)', () => {
  ...
  assert.deepStrictEqual(serverConfig.args, ['/path/to/server.js', '--port', '3000']);
});
...
test('E2E: Merge with existing config - preserves other servers', () => {
  ...
  installServerForAgent('server1', config1, 'cursor', { local: true, cwd: tempDir });
  
  // Second install
  const parsed2 = parseSource('mcp-server-postgres');
  const config2 = buildServerConfig(parsed2);
  installServerForAgent('server2', config2, 'cursor', { local: true, cwd: tempDir });
...
test('E2E: Overwrite existing server with same name', () => {
  ...
  const config1 = buildServerConfig(parsed1);
  installServerForAgent('myserver', config1, 'cursor', { local: true, cwd: tempDir });
  
  // Second install with same name but different URL
  const parsed2 = parseSource('https://mcp.new.com/api');
  const config2 = buildServerConfig(parsed2);
  installServerForAgent('myserver', config2, 'cursor', { local: true, cwd: tempDir });
...
test('E2E: Install to Goose (YAML format, transformed)', () => {
  ...
  assert.deepStrictEqual((transformed as Record<string, unknown>).args, ['-y', 'mcp-server-postgres']);
...
test('E2E: Zed config transformation - remote server', () => {
  ...
  const transformed = zedAgent.transformConfig!('example', config) as Record<string, unknown>;
...
test('E2E: Zed config transformation - local server', () => {
  ...
  const transformed = zedAgent.transformConfig!('postgres', config) as Record<string, unknown>;
...
test('E2E: Codex config transformation', () => {
  ...
  const transformed = codexAgent.transformConfig!('example', config) as Record<string, unknown>;
...
test('E2E: Codex config transformation - local server', () => {
  ...
  const transformed = codexAgent.transformConfig!('postgres', config) as Record<string, unknown>;
