 ...
 */
function transformGooseConfig(serverName: string, config: McpServerConfig): unknown {
  if (config.url) {
 ...
 */
function transformZedConfig(_serverName: string, config: McpServerConfig): unknown {
  if (config.url) {
 ...
 */
function transformOpenCodeConfig(_serverName: string, config: McpServerConfig): unknown {
  if (config.url) {
    return {
      type: 'remote',
      url: config.url,
 ...
 */
function transformCodexConfig(_serverName: string, config: McpServerConfig): unknown {
  if (config.url) {
    return {
      type: config.type || 'http',
      url: config.url,
...
export const agents: Record<AgentType, AgentConfig> = {
  ...
  codex: {
    name: 'codex',
    displayName: 'Codex',
    configPath: join(process.env.CODEX_HOME || join(home, '.codex'), 'config.toml'),
